...
class ProductListView(generic.ListView):
    ...
    def filter_by_price(self):
        ...
        if 'min_price' in self.request.GET:    
            ...
            if filter_price1 =='':
                filter_price1=0
            return qs.filter(price__range=(filter_price1,filter_price2))
    
    def search_by_title(self):
        qs = Product.objects.all()
        search_field = self.request.GET.get('search')
        if search_field:
            return qs.filter(Q(title__icontains=search_field))
        
        
    def get_context_data(self, **kwargs) -> Dict[str, Any]:
        context = super(ProductListView, self).get_context_data(**kwargs)
        context.update({"categories": Category.objects.values("name")})
        context['filtered'] = self.filter_by_price()
        context['searched'] = self.search_by_title()
